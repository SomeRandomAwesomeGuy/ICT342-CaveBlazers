...
public class SteamVR_Menu : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		if (cursor != null)
		{
			float x = Input.mousePosition.x, y = Screen.height - Input.mousePosition.y; // no gr3d warning
	...
	void Update()
	{
		if (Input.GetKeyDown(KeyCode.Escape) || Input.GetKeyDown(KeyCode.Joystick1Button7)) // no gr3d warning
		{
			if (overlay == null)
			{
				ShowMenu();
		...
		else if (Input.GetKeyDown(KeyCode.Home)) // no gr3d warning
		{
			SetScale(1.0f);
		}
		else if (Input.GetKey(KeyCode.PageUp)) // no gr3d warning
		{
			SetScale(Mathf.Clamp(scale + scaleRate * Time.deltaTime, scaleLimits.x, scaleLimits.y));
		}
		else if (Input.GetKey(KeyCode.PageDown)) // no gr3d warning
		{
			SetScale(Mathf.Clamp(scale - scaleRate * Time.deltaTime, scaleLimits.x, scaleLimits.y));
		}
	}
