...
public class getReal3D_Menu {
    ...
    static public void CheckForMultisampling()
    {
        ...
        for(int i=0; i<levelCount; ++i) {
            ...
            if(UnityEngine.QualitySettings.antiAliasing != 0) {
                string err = "Anti aliasing is activated for quality settings " +
        ...
        getRealCameraUpdater[] cameraUpdaters = Resources.FindObjectsOfTypeAll
            (typeof(getRealCameraUpdater)) as getRealCameraUpdater[];
    ...
    static public void ToogleUiDebug()
    {
        ...
        var wandEventModules = FindObjectsOfTypeInScene<WandEventModuleBase>();
        if(wandEventModules.Length != 1) {
            Debug.LogError(String.Format("Found {0} WandEventModule(s) in scene.",
                wandEventModules.Length));
        }
        ...
        if(standaloneInputModules.Length != 1) {
            Debug.LogError(String.Format("Found {0} StandaloneInputModule(s) in scene.",
                standaloneInputModules.Length));
        }

        Camera eventCamera = Array.Find(FindObjectsOfTypeInScene<Camera>(),
            cam => cam.gameObject.name == "MainCamera");
        if(eventCamera == null) {
